...
emitterObj.on('log', (msg, fileName) => logEvents(msg, fileName));
...
const serveFile = async (filePath, contentType, res) => {
    try {
        ...
        console.error(err);
        emitterObj.emit('log', `${err.name}: ${err.message}`, ERROR_LOG);
        res.statusCode = 500;
        res.end();
    }
};
...
const server = http.createServer((req, res) => {
    ...
    emitterObj.emit('log', `${req.url}\t${req.method}`, REQUEST_LOG);
    ...
    /**
     * Dynamically will append the .html extension, to each and every request when
